...
class FahrplanFragment : Fragment(), SessionViewEventsHandler {
    ...
    private fun observeViewModel() {
        ...
        viewModel.timeTextViewParameters.observe(viewLifecycleOwner) { timeTextViewParameters ->
            Logging.get().d(LOG_TAG, "getTimeTextViewParameters -> observe -> fillTimes")
            fillTimes(timeTextViewParameters)
        }
        viewModel.scrollToCurrentParameter.observe(viewLifecycleOwner) { (scheduleData, dateInfos) ->
     ...
     *
     * TODO Update docs - forceReload is no longer used
    ...
    private fun addRoomColumns(
        ...
        // TODO Track room names and check if they can be re-used with the updated scheduleData
