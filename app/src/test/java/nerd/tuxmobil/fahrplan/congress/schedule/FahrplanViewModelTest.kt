...
import nerd.tuxmobil.fahrplan.congress.schedule.observables.ScrollToCurrentParameter
...
class FahrplanViewModelTest {
    ...
    fun `fahrplanParameter neither posts to fahrplanParameter nor fahrplanEmptyParameter properties`() {
        ...
        assertLiveData(viewModel.fahrplanParameter).isEqualTo(null)
        assertLiveData(viewModel.fahrplanEmptyParameter).isEqualTo(null)
    ...
    @Test
    fun `scrollToCurrent never posts to scrollToCurrentParameter property when sessions is empty`() {
        val scheduleData = ScheduleData(0, emptyList())
        val repository = createRepository(uncanceledSessionsForDayIndexFlow = flowOf(scheduleData))
        val viewModel = createViewModel(repository)
        viewModel.scrollToCurrent()
        verifyInvokedNever(repository).readDisplayDayIndex()
        assertLiveData(viewModel.scrollToCurrentParameter).isEqualTo(null)
    }

    @Test
    fun `scrollToCurrent posts to scrollToCurrentParameter property when session is present and day indices match`() {
        ...
        val viewModel = createViewModel(repository)
        viewModel.scrollToCurrent()
        val expected = ScrollToCurrentParameter(
            scheduleData = createScheduleData(Session("session-31"), dayIndex = 3),
            dateInfos = DateInfos().apply { add(DateInfo(3, nowMoment)) }
        )
        assertLiveData(viewModel.scrollToCurrentParameter).isEqualTo(expected)
    }

    @Test
    fun `scrollToCurrent never posts to scrollToCurrentParameter property when day indices mismatch`() {
    ...
    fun `scrollToSession never posts to scrollToSessionParameter property when sessions is empty`() {
        ...
        assertLiveData(viewModel.scrollToSessionParameter).isEqualTo(null)
    }
